...
| Gate policy engine | `crates/tokmd-gate/src/evaluate.rs`, `crates/tokmd-gate/src/ratchet.rs`, and `crates/tokmd-gate/src/numeric.rs` | `evaluate.rs` 604; `ratchet.rs` 688; numeric owner 31 | Policy-rule evaluation and ratchet evaluation stay as public workflow owners while shared JSON numeric coercion lives in a private owner module; keep all gate engine changes under the `tokmd_gate` proof scope. |
