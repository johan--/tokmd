...
| Context packing | `crates/tokmd/src/context_pack.rs` and `crates/tokmd/src/context_pack/` | `context_pack.rs` 71; selection submodule/tests 1684; budget parser/tests 200 | Budget parsing and file selection now live in owner modules; continue splitting rendering and manifest helpers under `tokmd`; keep context/handoff proof scoped |
