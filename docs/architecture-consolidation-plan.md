...
| Format core outputs | `crates/tokmd-format/src/diff.rs`, `crates/tokmd-format/src/diff/`, `crates/tokmd-format/src/export.rs`, and `crates/tokmd-format/src/summary.rs` | `diff.rs` 534; diff compute owner 160; export 600; summary 616 | Diff row and total computation now lives in a dedicated owner module while rendering stays unchanged; future diff rendering, export, or summary splits should be snapshot-backed and preserve the root `tokmd-format` re-exports |
