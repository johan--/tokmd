...
| Gate command | `crates/tokmd/src/commands/gate.rs` and `crates/tokmd/src/commands/gate/` | `gate.rs` 229; policy owner/tests 215; render owner 111 | Gate policy loading, ratchet loading, and config-rule conversion live in the policy owner; text/JSON result rendering lives in the render owner; receipt computation remains in the command coordinator. Keep `tokmd_gate` proof scoped to all gate command paths. |
