...
| Format core outputs | `crates/tokmd-format/src/diff.rs`, `crates/tokmd-format/src/diff/`, `crates/tokmd-format/src/export.rs`, `crates/tokmd-format/src/export/`, and `crates/tokmd-format/src/summary.rs` | `diff.rs` 574; diff compute owner 168; export 490; export CSV owner 55; export CycloneDX owner 152; summary 687 | Diff row/total computation plus CSV and CycloneDX export rendering now live in dedicated owner modules while public helpers and output behavior stay unchanged; future JSON/JSONL export, diff rendering, or summary splits should be snapshot-backed and preserve the root `tokmd-format` re-exports |
