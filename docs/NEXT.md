...
- Next proof-policy operational slice: run the policy-default collector on `main` to confirm the checked four-scope promotion floor stays reproducible, then decide whether the collector should record a recency/window rule in policy before any required-gate or default Codecov-upload promotion.
