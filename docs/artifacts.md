...
| `proof-pack-route.json` | `target/ci/proof-pack-route.json` | `cargo xtask ci-plan --route-json-out <path>` | Changed files mapped to CI risk/proof packs, unmatched files, and lanes explicitly skipped by policy with lane kind, tier, blocking/expensive flags, and required labels. | It does not run proof commands, replace `affected.json`, or make skipped lanes passing evidence. | Inspect `unmatched_files` and `skipped_by_policy` before debugging broad CI proof. |
