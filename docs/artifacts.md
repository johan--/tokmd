...
| `ci-actuals.json` | `target/ci/ci-actuals.json` | `cargo xtask ci-actuals` from the `CI (Required)` aggregate job | Stable receipt over the aggregate job's `needs` payload, including each required job result and optional timing sidecar fields. | It does not change required gates, infer learned estimates, or turn skipped jobs into passing evidence. | Open with `target/ci/needs.json`; rerun `cargo xtask ci-actuals --needs target/ci/needs.json --output target/ci/ci-actuals.json`. |
